<section id="results" class="bg-white py-12 px-5 md:px-20 relative">
    <div>
        <header class="flex flex-col items-center gap-2 text-center mb-2 ">

            <h2 class=""><a href="/dentalone-results/">Our Results</a>
            </h2>
        </header>
        <div class="flex flex-col items-center gap-2 locations-description max-w-lg mx-auto mt-4">
            <p class="text-center ">At Dental<span class="text-primary">One</span>, we understand that every smile has its own story. Our skilled team is committed to enhancing your smile to its fullest potential. Explore our gallery of beautiful smile transformations and imagine the radiant, confident smile we can craft for you.
            </p>
            <a href="/results" class="link mt-4  animate__delay-1s">
                View results
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 inline-block ml-1">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 8.25L21 12m0 0l-3.75 3.75M21 12H3" />
                </svg>
            </a>
        </div>
    </div>
</section>